...
mu_t        = mu_a + mu_s;
g           = 0.75; % Scattering anisotropy
l_t_prime   = 1/(mu_a+mu_s*(1-g)); % Transport mean free path
d           = 0.02; % cm, depth of slab
...
parfor n = 1:N_packet
    ...
    while(1)
        ...
        % theory.
        if ((photon(1,3) > l_t_prime) && (photon(1,3) < d - l_t_prime) && (photon(1,6) > 0))
            photon(1,1:3) = photon(1,1:3) + l_t_prime/mu_t * photon(1,4:6);
